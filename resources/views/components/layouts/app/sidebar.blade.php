    ...
    <flux:sidebar sticky stashable class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:navlist variant="outline">
            <flux:navlist.group :heading="__('Platform')" class="grid">
                ...
                @if(auth()->user()->isRole('rrhh') || auth()->user()->isRole('admin'))
                <flux:navlist.item icon="home" :href="route('dashboard')" wire:navigate>
                    {{ __('Personal') }}
                </flux:navlist.item>
                @endif

                @if(auth()->user()->isRole('gestion') || auth()->user()->isRole('admin'))
                <flux:navlist.item icon="home" :href="route('dashboard')" wire:navigate>
                    {{ __('Gestión') }}
                </flux:navlist.item>
                    @if (auth()->user()->isRole('gestion'))
                    <flux:navlist.item icon="home" :href="route('dashboard')" wire:navigate>
                        {{ __('Planificacion') }}
                    </flux:navlist.item>
                    @endif
                ...
                auth()->user()->isRole('planificacion'))
                <flux:navlist.item icon="home" :href="route('dashboard')" wire:navigate>
                    {{ __('Planificacion') }}
                </flux:navlist.item>
